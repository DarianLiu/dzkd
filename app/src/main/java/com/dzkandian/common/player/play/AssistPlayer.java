...
public class AssistPlayer {
    ...
    private AssistPlayer() {
        mRelationAssist = new RelationAssist(MyApplication.get());
        mRelationAssist.setEventAssistHandler(mInternalEventAssistHandler);
        mRelationAssist.getSuperContainer().setBackgroundColor(Color.TRANSPARENT);
     ...
     * 设置视频展示尺寸
     *
     * @param aspectRatio 视频展示尺寸
     */
    public void setSourceSize(AspectRatio aspectRatio) {
        mRelationAssist.setAspectRatio(aspectRatio);
    }
    ...
    public void play(ViewGroup userContainer, DataSource dataSource) {
        ...
        if (receiverGroup != null && dataSource != null) {
            ...
            receiverGroup.getGroupValue().putBoolean(DataInter.Key.KEY_ERROR_SHOW, false);
        }

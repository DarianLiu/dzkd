...
public class ExplosionAnimator extends ValueAnimator {
    static long DEFAULT_DURATION = 0x600;
    ...
    public ExplosionAnimator(View container, @NonNull Bitmap bitmap, Rect bound) {
        mPaint = new Paint();
        mBound = new Rect(bound);

        int partLen = 22;//所有的粒子是15*15个
        mParticles = new Particle[partLen * partLen];
        Random random = new Random(System.currentTimeMillis());
        int w = bitmap.getWidth() / (partLen + 2);//Bitmap等分成了17*17个矩阵块
    ...
    @NonNull
    private Particle generateParticle(int color, @NonNull Random random) {
